...
class FileSystemDiff(object):
  def __init__(self, partition, tgt, src=None, threads=None, use_dynamic_partition=False):
    ...
    system_suffix = "system_root" if not use_dynamic_partition else "system"
  
    if self.partition == 'system':
      self.fs_root = "/%s" % (system_suffix + '/system')
    elif self.partition == 'root':
      self.fs_root = "/%s" % (system_suffix)
    else:
  ...
  def _Compute(self, root, tgt, src):
    ...
    print("Compute %s%s" % (self.fs_root, root))
