  ...
  """
    ...
    def GetBuildProp(self, prop):
        ...
        try:
            ...
            raise common.ExternalError("couldn't find %s in build.prop" %
                                       (prop, ))
    ...
    def CalculateFingerprint(self):
        ...
        return "%s/%s/%s:%s" % (self.GetOemProperty("ro.product.brand"),
  ...
  """
    def __init__(self):
        if OPTIONS.payload_signer is None:
            ...
            self.signer = "openssl"
            self.signer_args = [
                "pkeyutl", "-sign", "-inkey", signing_key, "-pkeyopt",
                "digest:sha256"
            ]
    ...
    def _GetKeySizeInBytes(signing_key):
        ...
        cmd = [
            "openssl", "rsa", "-inform", "PEM", "-in", signing_key, "-modulus",
    ...
    def Sign(self, in_file):
        ...
        cmd = [self.signer
               ] + self.signer_args + ['-in', in_file, '-out', out_file]
...
class Payload(object):
    ...
    """
        ...
        cmd = [
            "brillo_update_payload", "generate", "--payload", payload_file,
    ...
    """
        ...
        cmd = [
            ...
            self.payload_file, "--signature_size",
            str(payload_signer.key_size), "--metadata_hash_file",
            metadata_sig_file, "--payload_hash_file", payload_sig_file
        ]
        common.RunAndCheckOutput(cmd)
        ...
        signed_payload_file = common.MakeTempFile(prefix="signed-payload-",
                                                  suffix=".bin")
        cmd = [
            "brillo_update_payload", "sign", "--unsigned_payload",
            self.payload_file, "--payload", signed_payload_file,
            "--signature_size",
            str(payload_signer.key_size), "--metadata_signature_file",
            signed_metadata_sig_file, "--payload_signature_file",
            signed_payload_sig_file
        ]
        common.RunAndCheckOutput(cmd)
        ...
        properties_file = common.MakeTempFile(prefix="payload-properties-",
                                              suffix=".txt")
        cmd = [
            "brillo_update_payload", "properties", "--payload",
            signed_payload_file, "--properties_file", properties_file
        ]
        common.RunAndCheckOutput(cmd)
...
def _WriteRecoveryImageToBoot(script, output_zip):
  ...
  """
    ...
    recovery_two_step_img_path = os.path.join(OPTIONS.input_tmp, "IMAGES",
    ...
    if os.path.exists(recovery_two_step_img_path):
        ...
        common.ZipWriteStr(output_zip, recovery_two_step_img_name,
                           recovery_two_step_img.data)
        logger.info("two-step package: using %s in stage 1/3",
                    recovery_two_step_img_name)
...
def HasRecoveryPatch(target_files_zip):
    ...
    return ("SYSTEM/recovery-from-boot.p" in namelist
            or "SYSTEM/etc/recovery.img" in namelist)
...
def HasTrebleEnabled(target_files_zip, target_info):
    return (HasVendorPartition(target_files_zip)
            and target_info.GetBuildProp("ro.treble.enabled") == "true")
...
def WriteFingerprintAssertion(script, target_info, source_info):
    ...
    if source_oem_props is None and target_oem_props is None:
        script.AssertSomeFingerprint(source_info.fingerprint,
...
def AddCompatibilityArchiveIfTrebleEnabled(target_zip,
                                           output_zip,
                                           target_info,
                                           source_info=None):
  ...
  """
    ...
    AddCompatibilityArchive(system_updated or product_updated, vendor_updated
                            or odm_updated)
...
def CopyInstallTools(output_zip):
    ...
    for root, subdirs, files in os.walk(install_path):
        for f in files:
            ...
            install_target = os.path.join("install",
                                          os.path.relpath(root, install_path),
                                          f)
...
def GetBlockDifferences(target_zip, source_zip, target_info, source_info,
    ...
    def GetIncrementalBlockDifferenceForPartition(name):
        ...
        partition_src = common.GetUserImage(
            name,
            OPTIONS.source_tmp,
            source_zip,
        ...
        partition_tgt = common.GetUserImage(
            name,
            OPTIONS.target_tmp,
            target_zip,
        ...
        disable_imgdiff = (partition_source_info.fs_type == "squashfs"
                           or partition_target_info.fs_type == "squashfs")
        return common.BlockDifference(name,
                                      partition_src,
                                      partition_tgt,
                                      check_first_block,
    ...
    if source_zip:
        ...
        allow_shared_blocks = (
            source_info.get('ext4_share_dup_blocks') == "true"
            or target_info.get('ext4_share_dup_blocks') == "true")
        blockimgdiff_version = max(
            int(i)
            for i in target_info.get("blockimgdiff_versions", "1").split(","))
    ...
    for partition in partition_names:
        ...
        # Full OTA update.
        if not source_zip:
            tgt = common.GetUserImage(partition,
                                      OPTIONS.input_tmp,
                                      target_zip,
                                      info_dict=target_info,
                                      reset_file_map=True)
            block_diff_dict[partition] = common.BlockDifference(partition,
                                                                tgt,
                                                                src=None)
        # Incremental OTA update.
        else:
            block_diff_dict[
                partition] = GetIncrementalBlockDifferenceForPartition(
                    partition)
    ...
    if device_specific_diffs:
        ...
        for diff in device_specific_diffs:
            if diff.partition in block_diff_dict:
                logger.warning(
                    "Duplicate block difference found. Device specific block"
...
""" % bcb_dev)
    ...
    script.PrintPixelExperienceBanner(is_plus, android_version, build_id,
...
def GetPackageMetadata(target_info, source_info=None):
  ...
  """
    ...
    metadata = {
        ...
        target_info.GetBuildProp('ro.build.version.incremental'),
        'post-sdk-level':
        target_info.GetBuildProp('ro.build.version.sdk'),
        'post-security-patch-level':
        target_info.GetBuildProp('ro.build.version.security_patch'),
    }
...
def FinalizeMetadata(metadata, input_file, output_file, needed_property_files):
  ...
  """
    ...
    with zipfile.ZipFile(output_file) as output_zip:
        for property_files in needed_property_files:
            property_files.Verify(output_zip,
                                  metadata[property_files.name].strip())
...
def WriteBlockIncrementalOTAPackage(target_zip, source_zip, output_file):
    ...
    prev_build_date = source_info.GetBuildProp(
        "org.pixelexperience.build_date")
    prev_security_patch = source_info.GetBuildProp(
        "ro.build.version.security_patch")
    script.PrintPixelExperienceBanner(is_plus, android_version, build_id,
                                      build_date, security_patch, device,
                                      prev_build_id, prev_build_date,
                                      prev_security_patch)
...
def WriteFileIncrementalOTAPackage(target_zip, source_zip, output_file):
    ...
    if HasVendorPartition(target_zip):
        ...
        vendor_diff = common.FileSystemDifference("vendor", target_zip,
                                                  source_zip,
                                                  use_dynamic_partition)
    else:
        vendor_diff = None
    ...
    if HasProductPartition(target_zip):
        ...
        product_diff = common.FileSystemDifference("product", target_zip,
                                                   source_zip,
                                                   use_dynamic_partition)
    else:
        product_diff = None
    ...
    system_diff.WriteScript(script, output_zip, progress=0.8)

    root_diff.WriteScript(script, output_zip, progress=None)
...
def WriteABOTAPackageWithBrilloScript(target_file,
                                      output_file,
                                      source_file=None):
...
def main(argv):
    ...
    if OPTIONS.incremental_source is None:
        with zipfile.ZipFile(args[0], 'r') as input_zip:
            WriteFullOTAPackage(input_zip, output_file=args[1])
    ...
    # Generate an incremental OTA.
        ...
        with zipfile.ZipFile(args[0], 'r') as input_zip, \
            ...
            if OPTIONS.file_based:
                WriteFileIncrementalOTAPackage(input_zip,
                                               source_zip,
                                               output_file=args[1])
            else:
                WriteBlockIncrementalOTAPackage(input_zip,
                                                source_zip,
                                                output_file=args[1])
        ...
        if OPTIONS.log_diff:
            with open(OPTIONS.log_diff, 'w') as out_file:
                ...
                target_files_diff.recursiveDiff('', OPTIONS.source_tmp,
                                                OPTIONS.input_tmp, out_file)
