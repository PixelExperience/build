...
class Item(object):
  ...
  def SetPermissions(self, script):
    ...
    def recurse(item, current):
      # current is the (uid, gid, mode, selabel, capabilities) tuple
      ...
      if item.is_dir:
        ...
        if item.uid != current[0] or item.gid != current[1] or \
           item.mode != current[2] or item.selabel != current[3] or \
           item.capabilities != current[4]:
        ...
        if item.uid != current[0] or item.gid != current[1] or \
               item.mode != current[3] or item.selabel != current[3] or \
               item.capabilities != current[4]:
          script.SetPermissions("/"+item.name, item.uid, item.gid,
                                item.mode, item.selabel, item.capabilities)

    recurse(self, (-1, -1, -1, None, None))
...
class FileDifference(object):
  ...
  def EmitVerification(self, script, source_info):
    so_far = 0
    for tf, sf, _, _ in self.patch_list:
      if tf.name != sf.name:
        script.SkipNextActionIfTargetExists(tf.name, tf.sha1)
      part_type, part_device = common.GetTypeAndDevice("/"+self.partition, source_info)
      script.PatchPartitionCheck(
          "{}:{}:{}:{}".format(
              part_type, part_device, tf.size, tf.sha1),
          "{}:{}:{}:{}".format(
              part_type, part_device, sf.size, sf.sha1))
      so_far += sf.size
    return so_far
  ...
  def EmitPatches(self, script, total_patch_size, so_far):
    ...
    for item in self.patch_list:
      ...
      if sf.name != tf.name:
        script.SkipNextActionIfTargetExists(tf.name, tf.sha1)
      script.ApplyPatchIncrementalUpdate("/" + sf.name, "-", tf.size, tf.sha1, sf.sha1,
                        "patch/" + sf.name + ".p")
      so_far += tf.size
...
""" % bcb_dev)
  ...
  so_far = system_diff.EmitVerification(script, source_info)
  if vendor_diff:
    so_far += vendor_diff.EmitVerification(script, source_info)
