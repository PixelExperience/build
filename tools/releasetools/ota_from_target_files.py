...
class FileDifference(object):
  def __init__(self, partition, source_zip, target_zip, output_zip):
    ...
    for fn in sorted(target_data.keys()):
      ...
      zip_file_name = partition_prefix.upper()
      tf = target_data[fn]
      assert fn == tf.name
      ...
      if sf is None or tf.sha1 != sf.sha1:
        # File is different
        if tf.name.startswith(partition_prefix):
          zip_file_name += tf.name[len(partition_prefix):]
        d = target_zip.read(zip_file_name)
        common.ZipWriteStr(output_zip, partition_prefix + tf.name, d)
        if sf is not None:
...
def WriteIncrementalOTAPackage(target_zip, source_zip, output_file):
  ...
  script.Print("Your system can be updated!")
  ...
  script.Print("Patching system...")
  script.UnpackPackageDir("system", "/system")
  if vendor_diff:
    script.Print("Patching vendor...")
