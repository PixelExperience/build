...
class FileDifference(object):
  ...
  def EmitVerification(self, script):
    ...
    for tf, sf, _, _ in self.patch_list:
      ...
      script.FileCheck(sf.name, sf.sha1)
      so_far += sf.size
    return so_far
...
def WriteIncrementalOTAPackage(target_zip, source_zip, output_file):
  ...
  system_mount_point = script.fstab["/system"].mount_point
  script.AppendExtra("ifelse(is_mounted(\"{0}\"), unmount(\"{0}\"));".format(system_mount_point))
  script.AppendExtra("ifelse(is_mounted(\"{0}\"), unmount(\"{0}\"));".format("/mnt/system"))
  script.fstab["/system"].mount_point = "/mnt/system"
  script.Mount("/system", recovery_mount_options)

  if HasVendorPartition(target_zip):
    vendor_mount_point = script.fstab["/vendor"].mount_point
    script.AppendExtra("ifelse(is_mounted(\"{0}\"), unmount(\"{0}\"));".format("/mnt/vendor"))
    script.AppendExtra("ifelse(is_mounted(\"{0}\"), unmount(\"{0}\"));".format(vendor_mount_point))
    script.fstab["/vendor"].mount_point = "/mnt/vendor"
    script.Mount("/vendor", recovery_mount_options)

  if HasProductPartition(target_zip):
    product_mount_point = script.fstab["/product"].mount_point
    script.AppendExtra("ifelse(is_mounted(\"{0}\"), unmount(\"{0}\"));".format("/mnt/product"))
    script.AppendExtra("ifelse(is_mounted(\"{0}\"), unmount(\"{0}\"));".format(product_mount_point))
    script.fstab["/product"].mount_point = "/mnt/product"
    script.Mount("/product", recovery_mount_options)

  so_far = system_diff.EmitVerification(script)
  so_far += root_diff.EmitVerification(script)
  if vendor_diff:
    so_far += vendor_diff.EmitVerification(script)
  if product_diff:
    so_far += product_diff.EmitVerification(script)
  ...
  target_symlinks.extend(CopyPartitionFiles(root_items, target_zip, None))
  if vendor_diff:
  ...
  source_symlinks.extend(CopyPartitionFiles(root_items, source_zip, None))
  if vendor_diff:
